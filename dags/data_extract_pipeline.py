...
with DAG(
    ...
    schedule=None,
    start_date=datetime(2025, 5, 1, tzinfo=local_timezone),
    catchup=False,
    tags=["mlops", "study"],
) as dag:
    task1_sql = """
    ...
    task1 = SQLExecuteQueryOperator(
        task_id="create_database_temp", conn_id=conn_id, sql=task1_sql
    )
    ...
    task2 = SQLExecuteQueryOperator(
        task_id="drop_table_if_exists_temp.ineligible_loan_model_features_01",
        conn_id=conn_id,
        sql=task2_sql,
    )
    ...
    task3 = SQLExecuteQueryOperator(
        task_id="create_table_temp.ineligible_loan_model_features_01",
        conn_id=conn_id,
        sql=task3_sql,
    )
    ...
    task4 = SQLExecuteQueryOperator(
        task_id="drop_table_if_exists_temp.ineligible_loan_model_features_02",
        conn_id=conn_id,
        sql=task4_sql,
    )
    ...
    task5 = SQLExecuteQueryOperator(
        task_id="create_table_temp.ineligible_loan_model_features_02",
        conn_id=conn_id,
        sql=task5_sql,
    )
    ...
    task7 = SQLExecuteQueryOperator(
        task_id="insert_ineligible_loan_model_features", conn_id=conn_id, sql=task7_sql
    )
