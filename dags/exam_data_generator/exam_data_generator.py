...
from support.date_values import DateFormat, DateValues
...
def validate_exam_date(date_list, start_day, end_day):
    ...
    if len(date_list) != len(subtract_date_list):
        raise ValueError(
            "The data for the date range from 2023-06-01, to 2023-11-30, "
...
with DAG(
    ...
    data_generate = SQLExecuteQueryOperator.partial(
        ...
        params=get_date_params(
            start_day="{{ params.start_day }}", end_day="{{ params.end_day }}"
        )
    )
